...
export default function TimeBlocksPanel() {
  ...
  if (loading) return <p className="text-gray-500">Cargando bloques...</p>;

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-gray-900">Bloques de tiempo</h2>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <select
            value={doctorId}
      ...
      {blocks.length === 0 ? (
        <p className="text-gray-500">No hay bloques de tiempo registrados.</p>
      ) : (
              ...
              return (
                <div key={b.id} className="bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-gray-500 text-sm ml-3">{dateStr}</span>
                    <span className="text-gray-500 text-sm ml-2">{start} – {end}</span>
                  </div>
                  <button
