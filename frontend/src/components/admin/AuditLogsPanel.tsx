...
export default function AuditLogsPanel() {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 mb-4">Auditoría</h2>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {logs.map(log => (
            <div key={log.id} className="bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-between">
              <div>
                <span className={`font-medium ${roleColor[log.user.role] ?? 'text-gray-900'}`}>
                  {log.user.name}
                </span>
                <span className="text-gray-700 text-sm ml-3">{log.action}</span>
              </div>
